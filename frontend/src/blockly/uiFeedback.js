let currentHelpBalloon = null;

/**
 * Mostra uma notificação "toast" no canto da área de trabalho.
 * @param {Blockly.WorkspaceSvg} workspace O workspace principal do Blockly.
 * @param {string} message O texto a ser exibido.
 * @param {number} duration A duração em milissegundos.
 */
export function showToast(workspace, message, duration = 5000) {
 ...
 * @param {SVGElement} sourceElement O elemento SVG (o ícone) que o balão deve seguir.
 */
export function showHelpBalloon(text, sourceElement) {
    ...
    const closeListener = (event) => {
        if (currentHelpBalloon && currentHelpBalloon.contains(event.target)) {
            return;
        }
        if (sourceElement.contains(event.target)) {
            return;
        }
        if (currentHelpBalloon) {
            currentHelpBalloon.remove();
...
export function buildCommandHelpHTML(commandDefinition) {
    let html = `
